...
@Component({
  selector: 'app-super-targets',
  providers: [KpiService],
  templateUrl: './super-targets.component.html',
  styleUrls: ['./super-targets.component.css']
})
export class SuperTargetsComponent implements OnInit {
  ...
  ngOnInit() {
    this.kpiservice.ListAllCollection().subscribe((res: any) => {
      if (res.isSuccess) {
        ...
        this.AllData = this.AllData.filter(it => it.Role.toString() == '1');
        if (this.RoleID == '1')
